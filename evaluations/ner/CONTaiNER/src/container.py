...
def finetune_support(args, model, tokenizer, labels, pad_token_label_id):
    ...
    slow_lr=['bert']
    ori_lr = args.learning_rate_finetuning
    optimizer_grouped_parameters = [
                            {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay) and \
                                            not any(nd in n for nd in slow_lr) ], 'weight_decay': args.weight_decay,
                                                         'lr': ori_lr},
                                        {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay) and \
                                                         any(nd in n for nd in slow_lr) ], 'weight_decay': args.weight_decay,
                                                                      'lr': ori_lr *0.1},
...
def extract_target_labels(args, dataset, model):
    ...
    vecs, vecs_mu, vecs_sigma, labels = torch.tensor(np.asarray(fil_vecs)).to(args.device), torch.tensor(np.asarray(fil_vecs_mu)).to(args.device), torch.tensor(np.asarray(fil_vecs_sigma)).to(args.device), torch.tensor(np.asarray(fil_labels)).to(args.device)
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    if args.temp_trans > 0:
        ...
        for i in range(out_label_ids.shape[0]):
            sent_scores = torch.tensor(np.asarray(scores_list[i]))
...
def main():
    ...
    set_seeds(args)
    ori_out = args.output_dir
    ori_sp = args.support_path
    ori_test = args.test_path
    if args.do_train:
        imax = 1
    else:
        imax = 5000
    ...
    for i in range(imax):
        ...
        args.output_dir = ori_out + str(i)
        args.support_path = ori_sp + str(i)
        args.test_path = ori_test +str(i)
        if args.do_finetune_support_only:
            if args.silent == True:
                logging.disable(logging.ERROR)
            if args.saved_model_dir is None:
                #assume saved_model_dir is never None
