...
def best_score_by_task(log_file, scores_file, task, max_epoch=1000):
    with open(log_file, 'r', encoding='utf-8') as r:
        ...
        tasks = ['entity', 'mention', 'relation', 'trigger_id', 'trigger',
                 'role_id', 'role']
        for t in tasks:
            print('{}: dev: {:.2f}, test: {:.2f}'.format(t, best_scores[0][t]['f'] * 100.0, best_scores[1][t]['f'] * 100.0))
        
        with open(scores_file, 'w', encoding='utf-8') as f:
            for t in tasks:
                f.write('{}: dev: {:.2f}, test: {:.2f}'.format(t, best_scores[0][t]['f'] * 100.0, best_scores[1][t]['f'] * 100.0) + '\n')
